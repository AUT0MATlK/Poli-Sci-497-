...
setwd("/Users/samuelsouth/Downloads/TAD_2021-master/R lessons/wiki.RD")

wiki <- readRDS("~/Downloads/TAD_2021-master/R lessons/wiki.RDS")
file.choose()
...
#{r}
berlin <- word_vectors["america", , drop = FALSE] -
word_vectors["africa", , drop = FALSE] +
...
london <-  word_vectors["america", , drop = FALSE] -
word_vectors["africa", , drop = FALSE] +
word_vectors["canada", , drop = FALSE] +
word_vectors["U.S.", , drop = FALSE]
...
author<-  word_vectors["oregon", , drop = FALSE] - 
  word_vectors["ohio", , drop = FALSE] +
  word_vectors["oklahoma", , drop = FALSE]
cos_sim <- textstat_simil(as.dfm(word_vectors), y = as.dfm(author),
                          margin = "documents", method = "cosine")
head(sort(cos_sim[, 1], decreasing = TRUE), 5)
